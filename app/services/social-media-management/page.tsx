...
export async function generateMetadata({
  ...
  const canonical = `${BASE_URL}/services/social-media-management${slugPart}`;
...
type SocialMediaManagementPageProps = {
  Location?: string;
  searchParams?: { [key: string]: string | string[] | undefined };
};
...
export default function SocialMediaManagementPage({
  Location,
  searchParams,
}: SocialMediaManagementPageProps) {
  const raw = searchParams?.Location;
  const locationRaw = Array.isArray(raw) ? raw[0] : raw;
  ...
  const schemaBreadcrumb = breadcrumbSchema([
    ...
    { name: serviceData.title, url: "/services/social-media-management" },
  ]);
  return (
    <>
