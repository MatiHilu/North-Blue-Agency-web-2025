...
const serviceData = {
  ...
  process: [
    { title: "Audit", description: "We analyze your current digital situation" },
    { title: "Strategy", description: "We design a personalized comprehensive plan" },
    { title: "Execution", description: "We implement all planned tactics" },
    { title: "Optimization", description: "We continuously improve results" },
  ],
  benefits: [
...
export default function MarketingDigitalTemplate({ locationSlug }: { locationSlug: string }) {
  const locationText = normalizeLocation(locationSlug);
  const pageUrl = `/services/marketing-digital/${locationSlug}`;
  ...
  const schemaBreadcrumb = breadcrumbSchema([
    ...
    { name: serviceData.title, url: "/services/marketing-digital" },
