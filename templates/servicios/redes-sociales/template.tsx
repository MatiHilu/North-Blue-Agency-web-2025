...
const serviceData = {
  ...
  process: [
    { title: "Initial analysis", description: "We evaluate your current presence and define goals" },
    { title: "Strategy", description: "We create a personalized content plan" },
    { title: "Implementation", description: "We execute the strategy with quality content" },
    { title: "Optimization", description: "We adjust based on metrics and results" },
  ],
  benefits: [
...
export default function RedesSocialesTemplate({ locationSlug }: { locationSlug: string }) {
  const locationText = normalizeLocation(locationSlug);
  const pageUrl = `/services/redes-sociales/${locationSlug}`;
                ...
                <AnimatedSection key={index} delay={index * 100}>
                  <Card className="text-center border-0 shadow-lg hover:shadow-xl transition-shadow">
                    <CardContent className="p-6">
                      ...
                      <p className="text-gray-600 text-sm max-w-[200px] mx-auto">{step.description}</p>
                    </CardContent>
                  </Card>
                </AnimatedSection>
              ))}
